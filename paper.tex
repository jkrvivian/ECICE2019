...
However, MAM resolves this problem, it publishes messages to \textbf{channel}, which is a form of transactions that each address can be derived from the previous one. One can create multiple channels as possible, however, the size of a channel is fixed which is decided before creation, the more messages one wants to send in a channel, the more computation power is needed during creation. Thus data providers should decide how to distribute product into MAM channel by evaluating the cost. Furthermore, a session key can be provided to control the visibility and access to the data on MAM channel. 
